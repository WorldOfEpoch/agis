...
public class MorphEffect extends AgisEffect {
    ...
    public void apply(EffectState state) {
		...
		if (Log.loggingDebug)
			Log.debug("MorphEffect: apply effect params is: " + params);
		int claimID = -1;
		int objectID = -1;
		...
		if (claimID > 0 && objectID > 0) {
			Log.debug("MorphEffect: this effect is not for buildings");
			return;
		}	
		...
		if (target != null) {
			if (target.dead() && (target.getState() == null || (target.getState() != null && !target.getState().equals(CombatInfo.COMBAT_STATE_SPIRIT)))) {
				Log.debug("MorphEffect: target is dead");
    	...
    	if (removeOtherMorphs) {
        	...
        	for (EffectState existingState : target.getCurrentEffects()) {
        		if (existingState.getEffect() instanceof MorphEffect && !existingState.equals(state)) {
        			similarEffect = existingState;
        			if (existingState.getEffectID() == getID()) {
        				hasThisEffect = true;
        			}
        			Log.debug("MORPH: found similar effect, it is identical? " + hasThisEffect);
        			break;
        		}
        	}
    ...
    public void remove(EffectState state) {
		...
		for (AgisEffect.EffectState eState: getTargetEffectsOfMatchingType(target)) {
			...
			if (pEffect.getPriority() > highestPriority) {
				...
				Log.debug("MORPHEFFECT: found existing effect so setting " + propertyName + " to: " + value);
			}
		}
